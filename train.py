...
def train_net(model_fl,
    ...
    train_dataset_all = BasicDataset(csv_path, n_classes, transforms=train_transforms)
            ...
            if phase == 'train':
                with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
                    for batch in train_loader:
                        ...
                        loss_class_CE = L_class_CE(prediction, true_label_decode)

                        loss_class = loss_class_CE 
                        loss_class.backward()

                        running_loss += loss_class.item() * imgs.size(0)
                        running_corrects += torch.sum(prediction_decode == true_label_decode.data)
                ...
                with torch.no_grad():
                    ...
                    epoch_acc = running_corrects.double() / n_val
